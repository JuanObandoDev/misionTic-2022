...
const getDocByCategory = async (category) => {
  try {
    await connect();
    const docs = await db.collection("Documento").find({ Genero: category });
    return docs;
  } catch (error) {
    throw new Error("Error getting doc");
  }
...
const getDocByTitle = async (title) => {
  try {
    await connect();
    const docs = await db.collection("Documento").find({ Titulo: title });
    return docs;
  } catch (error) {
    throw new Error("Error getting doc");
  }
